...
COS 模块初始化，此函数应在所有函数之前调用
    ...
    global secret_id, secret_key, region, token, config, client,bucket,server_mode
    try:
    ...
    check_str = str(bytes_download("net_check.txt").decode("utf-8"))
    if check_str == "successful":
        server_mode = True
